...
| [evals/benchmark/README.md](evals/benchmark/README.md) | **Cost/quality benchmark** — runs the *whole* `/run-pipeline` on one frozen 9-scenario spec and scores the run against oracles the agents cannot see. Answers "what did this change cost, and did quality hold?", which no per-agent fixture can (findings 15, 16) |
| [evals/benchmark/bank-accounts/](evals/benchmark/bank-accounts/specification.md) | The **frozen spec** every arm runs verbatim — includes two deliberate *red-arrival chains* where a scenario is red only because its predecessor shipped without a guard |
| [evals/benchmark/run-arm.sh](evals/benchmark/run-arm.sh) | Materialise one arm; `--score` runs the oracles + scorecard. Refuses to overwrite an existing arm workspace |
| [evals/benchmark/oracle/](evals/benchmark/oracle/oracle.init.gradle.kts) | PIT + JaCoCo applied **out of band** via a Gradle init script, so agents never see a coverage or mutation plugin in the build they read |
| [evals/benchmark/rescore-cost.sh](evals/benchmark/rescore-cost.sh) | Re-derive every arm's cost side from stored transcripts without re-running the oracles (use after changing `extract_run.py`) |
| [evals/scorecard/extract_run.py](evals/scorecard/extract_run.py) | Transcript → normalised scorecard: API calls (keyed on `requestId`), tool calls per call, tokens, cache-read, generated chars by destination, and row-level quality |
...
| [docs/README.md](docs/README.md) | **Engineering findings (lab notebook)** — 17 measured discoveries: a grader bug that looked like model flakiness, skill-loading cost (~1.8×), the cost model, exhaustive corpora, generative-agent + integration + acceptance testing, orchestration-as-a-command (10), the test.json migration + cache decision (11), the v2 Spring/JPA vertical-slice integration (12), the strict-vs-batch-red TDD experiment (13), the mutation-gate spike (14), the pipeline cost programme (15), the rejected layered pipeline (16), and the rejected Bash-quieting rule with its two metric lessons (17) |
| **Other** | |
