...
- **Write side vs. read side (CQRS).** A use case orchestrates writes (commands that change state through aggregates with consistency boundaries). **Pure reads don't need a use case** — the controller may inject the read-side port directly. **Repository naming is reserved for write-side aggregates**; read-side ports are named **Finder / Query / Reader / Report**. Only introduce a read-side use case when there's actual logic on the way out (authorization, filtering, projection assembly). See `~/.claude/conventions/cqrs.md` for the full rules and litmus test.
