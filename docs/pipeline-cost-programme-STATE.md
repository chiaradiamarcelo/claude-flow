...
Immediate next action: **score arm `batching-only-3`, then decide PR #20.**

PR #17 was closed as superseded — it mixed the rig, findings 15/16, `spec-gap-reviewer`,
the Stage 3 reverts and BOTH turn-economy instructions into one unit, which is how #13
and #14 came to be measured together with neither attributable. Split into:

- **#19** `scorecard-per-request` — measurement only, no prompt change. This branch.
- **#20** `developer-batch-tool-calls` — the batching instruction alone (#13), stacked on
  #19, held until an admissible arm lands.
- **#14** Bash quieting returns to unshipped, its own PR and its own arm.
- **#21** new: a 529 destroys an arm (no backoff, four instant retries).

Queued after #20 is settled: **arm B** — one suite run per class instead of two, to cut
Gradle invocations (92 in the control) without losing behavioural red evidence. Gate
pre-registered: mutation candidate-real > 0 REJECTS. Then arm C (test+prod in one
message, no red) only if B disappoints; measured ceiling for C is <=16% of developer API
calls, and it would remove the property finding 14 relied on to justify having no
mutation gate.
...
cd runs/<arm> && nohup claude -p "/run-pipeline bank-accounts" --dangerously-skip-permissions > ../../<arm>-run.log 2>&1 < /dev/null &
...
hook rewrites `grep`, so `grep -c` does not return a bare integer and silently breaks
numeric comparisons.

**Never switch branches in `~/.claude` while an arm is running.** The checkout *is* the
live config the arm reads on every dispatch; a checkout mid-run silently changes the
treatment. To edit a file on another branch during an arm, use `git worktree add` to a
temporary path and commit from there.

**A 529 during an arm destroys it** — `/run-pipeline` retries the failed dispatch
immediately, four times, with no backoff, then continues as though the scenario were
merely skipped. That is issue #21. Until it is fixed, an arm that dies mid-run is more
likely upstream capacity than a config error: grep the subagent transcripts for
`isApiErrorMessage` before concluding anything about the treatment.
