...
class WithdrawMoneyController(
    ...
    ): WithdrawalResponse = responseFor(withdrawMoney.execute(accountId, request.amount))

    private fun responseFor(account: Account) = WithdrawalResponse(account.accountId(), account.balance())
}
